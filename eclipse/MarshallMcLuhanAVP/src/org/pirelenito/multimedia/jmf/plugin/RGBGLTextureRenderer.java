...
public class RGBGLTextureRenderer implements VideoRenderer, Control, IGLTextureRenderer {
	...
	private Texture createTexture(GL gl, ByteBuffer byteBuffer, int width, int height) {
		...
		Texture texture = TextureIO.newTexture(new TextureData (gl.getGLProfile(), GL.GL_RGB,width, height, 0, GL.GL_RGB, GL.GL_UNSIGNED_BYTE, false, false, true, byteBuffer, null));
		texture.enable();
	...
	public int process(Buffer buffer) {
		...
		if (byteBuffer == null) {
			byteBuffer = Buffers.newDirectByteBuffer(buffer.getLength() * 3);
