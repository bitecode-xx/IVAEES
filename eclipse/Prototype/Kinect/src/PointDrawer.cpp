...
void XnVPointDrawer::OnPointUpdate(const XnVHandPointContext* cxt)
{
	...
	//
	char* action = "test\n";
	kc->sendData((double)ptProjective.X, (double)ptProjective.Y, (double)ptProjective.Z, 1, action);
}
...
void PrintSessionState(SessionState eState)
{
	...
	{
	case IN_SESSION:
		sprintf_s(strLabel, "Tracking hands"); break;
	case NOT_IN_SESSION:
		sprintf_s(strLabel, "Perform click or wave gestures to track hand"); break;
	case QUICK_REFOCUS:
		sprintf_s(strLabel, "Raise your hand for it to be identified, or perform click or wave gestures"); break;
	}
