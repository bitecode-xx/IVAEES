...
public class BGRAGLTextureRenderer implements VideoRenderer, Control, IGLTextureRenderer {
	...
	private Texture createTexture(GL gl, IntBuffer byteBuffer, int width, int height) {
		...
		Texture texture = TextureIO.newTexture(new TextureData (gl.getGLProfile(),GL.GL_RGBA8,width, height, 0, GL2.GL_BGRA, GL2.GL_UNSIGNED_INT_8_8_8_8_REV, false, false, true, byteBuffer, null));
		texture.enable();
	...
	public boolean render(GL gl) {
		...
		synchronized (byteBuffer) {
			...
			if (texture == null) {
				...
				texture.bind();		
				gl.glTexSubImage2D(texture.getTarget(), 0, 0, 0, width, height, GL2.GL_BGRA, GL2.GL_UNSIGNED_INT_8_8_8_8_REV, byteBuffer);
			}
	...
	public int process(Buffer buffer) {
		...
		if (byteBuffer == null) {
			byteBuffer = Buffers.newDirectIntBuffer(buffer.getLength());
